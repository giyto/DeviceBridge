...
Android и web UI MUST использовать один смысл status colors, typography hierarchy и primary/secondary/destructive actions для connection, pairing, text, file, history и settings. Loading, empty, error, success и cancellation surfaces MUST сохранять структуру экрана и MUST NOT вызывать неожиданный layout shift, потерю focus или появление действия за границами контейнера.
