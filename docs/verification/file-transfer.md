...
- Result: the native-download plus explicit post-download SHA-256 workflow is feasible; production implementation may continue without weakening the specification.
...
- Playwright selects the browser-saved file through an ordinary `input type=file`.
- The fixture reads `File.stream()` incrementally and uses the same `StreamingSha256` adapter intended for the production web client.
- A wrong file produces `mismatch` and sends no acknowledgement.
- A matching size and SHA-256 sends exactly one acknowledgement and produces `verified`.

### Incremental SHA-256 selection
...
- Browser verification processed a deterministic 500 MiB stream from one reused 1 MiB chunk. Both browsers produced `47c2e1d3068c1e31e17d7d6a2787f22ce486bc224772bbdc54fff13598306290`.
...
- Date: 2026-09-17.
- Web unit/contract result: 26 test files and 119 tests passed.
- TypeScript and Vite production build: passed; the generated JavaScript bundle is 66.47 kB raw / 19.27 kB gzip and the stylesheet is 15.20 kB raw / 3.79 kB gzip.
...
- Chrome/Edge file-transfer gate: 8/8 tests passed. Native download, explicit verification, expired/reused grants and session-page continuity passed in both branded browser channels.
- Deterministic browser SHA-256 fixture: both Chrome and Edge produced `47c2e1d3068c1e31e17d7d6a2787f22ce486bc224772bbdc54fff13598306290` for 500 MiB. Chrome elapsed 8,398.2 ms and Edge elapsed 8,385.8 ms; sampled JS heap growth was 0 bytes in both runs.
- Android JVM result: 343 unit tests, `assembleDebug` and Android test compilation passed.
...
- Coordinator tests cover queued, transferring and verifying cancellation, active resource closure order, browser disconnect, session revoke, network failure, server stop, queue recovery and 20 repeated cancel/stop cycles.
- Upload, download and control route tests cover premature/invalid length, owner isolation, pre-approval denial, checksum mismatch, duplicate verification, expired/single-use grant and source/target closure.
