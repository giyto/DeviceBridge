...
test("file cancel and retry keep keyboard focus on the same transfer", async ({ page }) => {
  await page.addInitScript(() => {
    sessionStorage.setItem("devicebridge.session.v1", "keyboard-file-token");
  });
  await page.route("**/web-manifest.json", (route) => route.fulfill({
    contentType: "application/json",
    body: JSON.stringify({ protocolVersion: 1, webAssetVersion: "sha256-keyboard-test" }),
  ...
  const metadata = {
    transferId: "focus-file",
    displayName: "keyboard-focus.txt",
    sizeBytes: 4,
    mimeType: "text/plain",
    sha256: "a".repeat(64),
    direction: "ANDROID_TO_BROWSER",
  };
  const snapshot = (status: "TRANSFERRING" | "CANCELLED" | "CONNECTING", id: string) => ({
    protocolVersion: 1,
    messageId: id,
    type: "file.snapshot",
    timestamp: 4_000,
    items: [{ metadata, status, bytesTransferred: 0, speedBytesPerSecond: 0 }],
  });
  await page.route("**/api/v1/transfers/focus-file", (route) => {
    expect(route.request().method()).toBe("DELETE");
    return route.fulfill({
      contentType: "application/json",
      body: JSON.stringify(snapshot("CANCELLED", "cancelled-keyboard")),
    });
  });
  await page.route("**/api/v1/transfers/focus-file/retry", (route) => {
    expect(route.request().method()).toBe("POST");
    return route.fulfill({
      contentType: "application/json",
      body: JSON.stringify(snapshot("CONNECTING", "retry-keyboard")),
    });
  });
  await page.routeWebSocket("**/api/v1/events", (socket) => {
    socket.onMessage((message) => {
      ...
      socket.send(JSON.stringify({
        protocolVersion: 1,
        messageId: "authenticated-keyboard",
        type: "session.authenticated",
        timestamp: 4_000,
      }));
      socket.send(JSON.stringify(snapshot("TRANSFERRING", "snapshot-keyboard")));
    });
  });

  await page.goto("/");
  const card = page.locator('[data-transfer-id="focus-file"]');
